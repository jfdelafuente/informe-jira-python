...
'''
Funcion que vuelca al fichero cuatro campos: NumIncidencia, Bug, Status y Resolucion
'''
def crear_fichero_cuatro_campos(texto):
...
'''
def crear_fichero_tres_campos(texto):
    if texto["maxResults"] > texto["total"]:
        contar = texto["total"]
    else:
        contar = texto["maxResults"]
...
def main():
    ...
    df_epsilons = pd.read_excel(configD.DIR_JIRA_IN + archivo)
    estatus, texto = funcionUnaConsulta(jira, df_epsilons)

    with open(configD.DIR_JIRA_OUT + "orders_new.json", 'w') as file:
        json.dump(texto, file)
        
    if estatus == 200:
        df_salida = pd.DataFrame()
        lista_inc, lista_bug, lista_status, lista_resolution = crear_fichero_tres_campos(texto)
