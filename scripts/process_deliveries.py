...
def main():
    ...
    output.header("PROCESAMIENTO ETL - DELIVERIES")
    ...
    try:
        ...
        if extracted_delivs.empty:
            ...
            output.warning("No se encontraron archivos JSON de deliveries para procesar")
            output.info(f"Verifica que existan archivos en: {Config.DELIVS_JSON_DIR}")
            return 1

        logger.info(f"Extraídos {len(extracted_delivs)} registros")
        output.success(f"Extraidos {len(extracted_delivs)} registros")
        ...
        output.success(f"Transformados {len(transformed_data)} registros")
        ...
        stats = {
            'processed': len(extracted_delivs),
            'success': len(transformed_data),
            'duration': duration,
            'location': str(output_file)
        }

        output.summary(stats, "RESUMEN FINAL - PROCESAMIENTO DELIVERIES")
